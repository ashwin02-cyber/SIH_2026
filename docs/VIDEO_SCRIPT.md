...
4. Open these in extra browser tabs / windows, ready to show: `docs/sample_reports/sample-executive.pdf`, `ml-engineer/confusion_matrix.png`, `docs/architecture.png`. Optional extras if you have time (they do not fit the 3:00 script): the **Defence what-if** panel and **Replay** checkbox, both labelled as simulation / replay.
...
| 5 | 1:35-2:05 | Scroll to **Traffic classification**, the **inferred** badges in the findings list, **Recommendations** (open the strongSwan snippet), **Traffic timeline** | The model recognises what is inside the tunnel, or says unrecognised when it is unsure. From packet sizes alone the tool also infers the cipher family and tunnel mode, and checks sequence numbers for replay evidence. Then it lists prioritised fixes with a generated strongSwan snippet, marked as not yet tested. | Sizes only · never guessed |
